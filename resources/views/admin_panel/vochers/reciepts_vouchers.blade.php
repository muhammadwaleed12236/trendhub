    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change', '.rowAccountHead', function() {
                ...
                $subSelect.html('<option disabled selected>Loading...</option>');
                let fetchUrl = '{{ route("get.accounts.by.head", ":id") }}'.replace(':id', headId);
                $.get(fetchUrl, function(res) {
                    ...
                    if (res && res.length > 0) {
                        ...
                        html += '<option disabled>No accounts found</option>';
                    }
                    $subSelect.html(html);
                }).fail(function() {
                    $subSelect.html('<option disabled selected>Error loading accounts</option>');
                });
            });
