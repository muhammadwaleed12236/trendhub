@foreach ($sales as $sale)
    ...
    <tr class="d-table-row d-md-none border-0">
        <td colspan="12" class="p-0 border-0 bg-transparent">
            <div class="sale-mcard p-3 bg-white rounded-3 border mb-3 shadow-sm">
                <div class="d-flex align-items-center justify-content-between mb-2">
                    <div class="d-flex align-items-center gap-2">
                        <span class="fw-bold text-dark fs-6">#{{ $sale->reference ?? $sale->id }}</span>
                    </div>
                    <div>{!! $statusBadge !!}</div>
                </div>

                <div class="d-flex align-items-center gap-2 my-2">
                    <div class="avatar-circle text-info fw-bold d-flex align-items-center justify-content-center rounded-circle" style="width: 32px; height: 32px; font-size: 13px; background-color: #e0f2fe; color: #0369a1;">
                        {{ strtoupper(substr(optional($sale->customer_relation)->customer_name ?? 'C', 0, 1)) }}
                    </div>
                    <div>
                        <div class="fw-bold text-dark small">{{ optional($sale->customer_relation)->customer_name ?? 'N/A' }}</div>
